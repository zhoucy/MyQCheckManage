...
<script type="text/javascript">
  var groupid='<%=session.getAttribute("groupid")%>';
			...
			if (validateForm2().form()) {
				$.ajax({
					...
					success : function(data) {
						...
						if (array) {
							var tr = $("<tr></tr>");
							var td1 = $("<td id='checkid'>" + array.id + "</td>");
							var td2 = $("<td>" + array.name + "</td>");
							var td3 = $("<td> <a href='javascript:delRecord(\""+data.recordid+"\");'>删除</td>");
    ...
    if(confirm("确定要删除该记录？")){
        ...
        var url = "delRecord.do?id="+id;
        $.get(url,function(data){
             alert(data);
             if(data=="删除成功"){
                //合计数-1
	            if($("table tr td ").eq(0).html()=="P01"){
	               var goodNum= Number($("#goodNum").text())-1;
	               $("#goodNum").text(goodNum);
	            }else{
	              $("#badNum").text(Number($("#badNum").text())-1);  
	            }
	               $("#tby tr").remove();
            }
        });
	...
	$(function($) {
		...
		$("#progress").change(
				function() {
					if ($("#progress").val() == 1) {
						$("#checkProjectTable").hide();
						$("#QCCheckProjectID").hide();
						$("#packbarcode").show();
						$("#submitTest").hide();
						$("#submitPackage").show();
						$("#pid").html("产品编码：");
						...
						$("#QCCheckProjectID").show();
						$("#packbarcode").hide();
						$("#checkProjectTable").show();
						$("#submitTest").show();
						$("#submitPackage").hide();
						$("#pid").html("项目编码：");
					}
					...
					var url = "${ctx}/qc/getProductNum.do?productid=" + productid
		...
		$("#mainform").keydown(function(e) {
			...
			if (keycode == 13&&$("#QCCheckProjectID").val()!=null&&$("#QCCheckProjectID").val()!="") {
				
				$("#submitTest").trigger("click");
				return false;
			}
			if (keycode == 13&&$("#packbarcode").val()!=null&&$("#packbarcode").val()!="") {
				
				$("#submitPackage").trigger("click");
				return false;
			}
		...
		<div class="g-main">
			<div class="g-left">
				<dl>
					...
					<dd>	<a href="${ctx}/qc/QCCheckTest.jsp"  >装箱检测</a></dd>
				</dl>
			</div>
			<div class="g-right margin-left30">
				...
				<div class="configureBox">
					<form id="mainform" name="mainform" method="post">
						...
						<input type="hidden" class="input-1p Wdate left width150" name="productid" id="productid"/>
						<label class="label-1 left">生产订单：</label>
						<input type="text" class="input-1p Wdate left width150" name="pono" id="pono"/>
						<span class="left "></span>
						<label class="label-1 left">产品型号：</label> 
						<input type="text" class="input-1p Wdate left width150" name="model" id="model" readonly="readonly" />
						<span class=" left "></span> 
						<label class="label-1 left">产品类型：</label> 
						<select name="prodectType"
						...
						<label class="label-1 left">目标产能：</label> <input type="text"
							...
							id="standardProduct"><span class=" left "></span>
						<label id="pid" class="label-1 left">项目编码：</label> 
						<input type="text"class="input-1p Wdate left width150" name="qccheckprojectid" id="QCCheckProjectID" />
						<span ></span>
						<input type="text"class="input-1p Wdate left width150" name="packbarcode" id="packbarcode" />
						<span class="left "></span>
						<div class="blank20"></div>
						<div class="a-btn-1 left">
							<input type="button" id="submitTest" value="确定"> <input
								type="button" id="submitPackage" value="确定a">
						</div>
						<div class="a-btn-1 left">
				...
				<div class="blank20"></div>
				<div class="list-main">
					<table class="gridtable" id="checkProjectTable">
						<thead>
							<tr height="40px">
								<th width="8%">项目编码</th>
								<th width="8%">项目名称</th>
